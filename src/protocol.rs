...
impl DacStatus {
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        v.push(self.source);
        v.write_u16::<LittleEndian>(self.light_engine_flags)
            .unwrap();
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let display = match *self {
            Command::Begin {
                ...
                "Begin: low_water_mark {} point_rate {}",
                low_water_mark, point_rate
            ),
            Command::ClearEStop => "ClearEStop".to_string(),
            Command::Data { num_points, .. } => format!("Data: num_points {}", num_points),
            Command::EmergencyStop => "EmergencyStop".to_string(),
            Command::Ping => "Ping".to_string(),
            Command::Prepare => "Prepare".to_string(),
            Command::QueueRateChange => "QueueRateChange".to_string(),
...
impl DacResponse {
    pub fn new(response: ResponseState, command: u8, dac_status: DacStatus) -> DacResponse {
        DacResponse {
            response: response,
            command: command,
