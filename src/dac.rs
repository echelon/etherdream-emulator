...
impl Dac {
    ...
    pub fn listen(&self) -> Result<(), EmulatorError> {
        ...
        loop {
            ...
            match command {
                Command::Begin { .. } => {
                    self.write(&mut stream, &command).unwrap();
                }
                Command::Prepare => {
                    self.write(&mut stream, &command).unwrap();
                }
                ...
                Command::Version => {
                    self.write_version(&mut stream).unwrap();
                }
                _ => {
    ...
    fn read_command(&self, stream: &mut TcpStream) -> Result<Command, EmulatorError> {
        ...
        match buf[0] {
            ...
            COMMAND_VERSION => Ok(Command::Version),
            _ => {
    ...
    fn write(&self, stream: &mut TcpStream, command: &Command) -> Result<(), EmulatorError> {
        let status = self.status.read()?.clone();
        let response = &DacResponse::new(ResponseState::Ack, command.value(), status).serialize();
        let _size = stream.write(response)?;
...
mod tests {
    ...
    fn test_version_command() {
        ...
        thread::sleep(Duration::from_millis(250)); // Wait for DAC to accept connections.
        ...
        let expected: Vec<u8> = vec![
            // 'v0.0.1'
            0x76, 0x30, 0x2E, 0x30, 0x2E, 0x31, // 26 bytes padding
    ...
    fn assert_ack(stream: &mut TcpStream, cmd_byte: u8) {
        ...
        // ack + command byte
        let expected: Vec<u8> = vec![
            0x61, cmd_byte, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
